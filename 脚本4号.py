    ...
    def run_server(self):
        ...
        class RequestHandler(BaseHTTPRequestHandler):
            ...
            def _send_classify_page(self):
                """发送图片识别页面"""
                # 检查Pillow库是否安装，生成相应的警告信息
                if not HAS_PILLOW:
                    pillow_warning = '''
                    <div class="result-box" style="border-left-color: #f44336; background: #ffebee;">
                        <h3 style="color: #f44336; margin-bottom: 10px;">⚠️ 警告: 图片功能受限</h3>
                        <p>Pillow图像处理库未安装，无法使用图片识别功能。</p>
                        <p>请安装Pillow库: <code>pip install Pillow</code></p>
                    </div>
                    '''
                else:
                    pillow_warning = ""
            ...
            def _handle_upload(self):
                ...
                try:
                    ...
                    try:
                        ...
                        # 如果图像分析失败，提供友好的错误信息
                        results_html = f'''
                        <div class="result-box" style="border-left-color: #ff6b6b; margin-top: 20px;">
                            <h3>❌ 识别失败</h3>
                            <p><strong>错误信息：</strong> {str(e)}</p>
                            <p>请确保上传的是有效的图片文件（JPG/PNG格式），且文件大小不超过5MB</p>
                        </div>
                        '''
